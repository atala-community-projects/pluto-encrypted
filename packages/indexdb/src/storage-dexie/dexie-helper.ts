...
import { type RxDocumentData, type RxJsonSchema, RxStorageDefaultStatics, getPrimaryFieldOfPrimaryKey, getSchemaByObjectPath, newRxError, toArray } from 'rxdb'
 ...
 *
 */
export interface DexieStorageInternals {
  dexieDb: Dexie
  /**
...
const DEXIE_STATE_DB_BY_NAME = new Map<string, DexieStorageInternals>()
const REF_COUNT_PER_DEXIE_DB = new Map<DexieStorageInternals, number>()
...
export async function getDocsInDb<RxDocType> (
  ...
  docIds: string[]
): Promise<Array<RxDocumentData<RxDocType>>> {
